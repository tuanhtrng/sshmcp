...
concept tool = requires(context_t& context, nlohmann::json const& arguments) {
    { ToolType::NAME } -> std::convertible_to<std::string_view>;
    { ToolType::DESCRIPTION } -> std::convertible_to<std::string_view>;
    { ToolType::schema() } -> std::same_as<nlohmann::json>;
    { ToolType::invoke(context, arguments) } -> std::same_as<tool_result_t>;
};
...
class tool_set_t {
  public:
    static auto list() -> nlohmann::json {
        ...
        (tools.push_back({{"name", ToolTypes::NAME},
    ...
    static auto call(context_t& context, std::string_view name,
        ...
        ((name == ToolTypes::NAME
              ? (result = ToolTypes::invoke(context, arguments), true)
              : false) ||
         ...);
        return result;
    }
