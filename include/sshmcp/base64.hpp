...
inline auto base64_encode(std::string_view data) -> std::string {
    ...
    while (index + 3 <= data.size()) {
        ...
        auto const b = static_cast<unsigned char>(data[index + 1]);
        auto const c = static_cast<unsigned char>(data[index + 2]);
        out.push_back(BASE64_ALPHABET[a >> 2]);
        out.push_back(BASE64_ALPHABET[((a & 0x03) << 4) | (b >> 4)]);
        out.push_back(BASE64_ALPHABET[((b & 0x0f) << 2) | (c >> 6)]);
...
inline auto base64_decode(std::string_view text)
    ...
    if (text.size() % 4 != 0) {
        return std::unexpected{error_t{"base64: length not multiple of 4"}};
    }
    auto out = std::string{};
    out.reserve(text.size() / 4 * 3);
    for (auto index = std::size_t{0}; index < text.size(); index += 4) {
        ...
        for (auto slot = std::size_t{0}; slot < 4; ++slot) {
            ...
            if (ch == '=') {
                if (!is_last || slot < 2 || (slot == 2 && quad[3] != '=')) {
                    return std::unexpected{error_t{"base64: bad padding"}};
                }
                ++pads;
                values[slot] = 0;
            } else {
        ...
        if (pads < 2) {
            out.push_back(static_cast<char>(((values[1] & 0x0f) << 4) |
                                            (values[2] >> 2)));
        }
        if (pads < 1) {
            out.push_back(
