...
inline auto posix_spawn_capture(spawn_request_t const& request)
    ...
    while (out_open || err_open) {
        ...
        auto const wait_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(deadline -
                                                                  now)
                .count();
        pollfd fds[3] = {{in_open ? in_pipe[1] : -1, POLLOUT, 0},
        ...
        if (in_open && (fds[0].revents & (POLLOUT | POLLERR))) {
            auto const n =
                write(in_pipe[1], request.stdin_data.data() + stdin_offset,
                      request.stdin_data.size() - stdin_offset);
            if (n > 0) {
                stdin_offset += static_cast<std::size_t>(n);
            }
            if (n <= 0 || stdin_offset >= request.stdin_data.size()) {
                close(in_pipe[1]);
                in_open = false;
            }
        }
        if (out_open && (fds[1].revents & (POLLIN | POLLHUP))) {
            auto const n = read(out_pipe[0], buffer, sizeof(buffer));
            if (n <= 0) {
                close(out_pipe[0]);
                out_open = false;
            } else {
                stdout_text.append(buffer, static_cast<std::size_t>(n));
            }
        }
        if (err_open && (fds[2].revents & (POLLIN | POLLHUP))) {
            auto const n = read(err_pipe[0], buffer, sizeof(buffer));
            if (n <= 0) {
                close(err_pipe[0]);
                err_open = false;
            } else {
    ...
    auto const exit_code = WIFEXITED(status) ? WEXITSTATUS(status) : 128;
