...
struct exec_tool_t {
    ...
    static auto invoke(context_t& context,
        ...
        auto const out = truncate_middle(spawned->stdout_text,
                                         MAX_OUTPUT_CHARS);
        auto const err = truncate_middle(spawned->stderr_text,
                                         MAX_OUTPUT_CHARS);
    ...
    static auto invoke(context_t& context,
        ...
        if (spawned->stdout_text.size() > MAX_READ_BYTES) {
            return error_result(std::format(
                "read_file: {} exceeds 1 MiB ({} bytes)", path,
...
struct write_file_tool_t {
    ...
    static auto invoke(context_t& context,
        ...
        if (!arguments.contains("content") ||
            ...
            return error_result(
                "write_file: 'content' (string) is required");
        }
        auto const path = arguments["path"].get<std::string>();
        ...
        return {.text = std::format("wrote {} bytes to {}",
                                    size, path),
                .is_error = false};
    }
};
