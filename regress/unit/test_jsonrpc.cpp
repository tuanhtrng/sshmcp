...
[[maybe_unused]] auto const t1 = t::add_test("parse request", [] {
    auto const request =
        sshmcp::parse_request(R"({"jsonrpc":"2.0","id":7,"method":"ping"})");
    ...
    t::expect(request->params.is_object(), "params normalized to object");
});
...
[[maybe_unused]] auto const t3 = t::add_test("bad input", [] {
    t::expect(!sshmcp::parse_request("not json").has_value(), "invalid json");
    t::expect(!sshmcp::parse_request(R"({"id":1})").has_value(),
              "missing method");
    t::expect(!sshmcp::parse_request(R"({"jsonrpc":"1.0","id":1,"method":"m"})")
                   .has_value(),
              "wrong version");
});
...
[[maybe_unused]] auto const t4 = t::add_test("serialize", [] {
    auto const result =
        nlohmann::json::parse(sshmcp::make_result(3, {{"ok", true}}));
    ...
    auto const error = nlohmann::json::parse(
        sshmcp::make_error(nlohmann::json{}, sshmcp::PARSE_ERROR, "bad"));
    t::expect(error["error"]["code"] == sshmcp::PARSE_ERROR, "error code");
    t::expect(error["error"]["message"] == "bad", "error message");
